...
### Build Brief v2 candidate protocol

This additive candidate inventory applies only to the Specifications API v2 and
Build Brief v2 candidates. The unversioned inventory and existing v1 runtime remain
unchanged. This document does not adopt the candidate contracts, publish an API or
qualify signing, authorization or storage providers.

**Commands v2 candidate:** `CreateSpecWorkspace`, `AddRequirement`, `RecordDecision`, `ResolveDecision`, `AttachContract`, `DefineAcceptanceCriterion`, `SubmitSpecForReview`, `ReviewSpec`, `AcceptSpecPackage`, `SupersedeSpecPackage`, `CreatePlanningHandoff`, `ExportSpecPackage`.

**Queries v2 candidate:** `GetSpecWorkspace`, `ListOpenDecisions`, `ValidateSpecPackage`, `DiffSpecVersions`, `GetAcceptedPackage`, `GetHandoff`, `GetApprovalHistory`, `GetAcceptanceSubject`.

`GetAcceptanceSubject` reads the complete canonical submitted Build Brief body and
its independently recomputable JCS/SHA-256 content digest before the first detached
signature. The response also exposes the current aggregate revision, outside the
digest preimage, for the subsequent acceptance command's `If-Match` check. The body
contains the package identity/version reserved by the trusted ownership authority;
neither is inferred from the workspace identity or mutable aggregate revision.

The candidate HTTP projection is GET on the existing workspace acceptance route.
It resolves workspace-to-package ownership independently and requires the existing
dedicated `spec-package` / `read` permission with a current authenticated session
and membership. It grants no workspace role. Only a complete submitted or accepted
subject is returned; unavailable authority, draft or superseded state, foreign
ownership, invalid bytes or an unverified digest refuse without a signing subject.
An approver recomputes the digest and verifies the intended organization, package
identity/version and complete body before signing through their independently
qualified signing authority. Reading the subject is not a contribution or approval.

The acceptance command checks the same subject and current revision atomically,
revalidates every detached acceptance and independent contributor provenance, and
refuses a stale subject. Current read access cannot become approval permission.
No public/private key is accepted from this query, no server signature is produced,
and no new persisted subject snapshot, retention class or interval is introduced.
Responses are bounded, canonical and non-cacheable under the v2 HTTP contract.
The existing accepted-package query remains a read of an already accepted package.
